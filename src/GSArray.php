...
class GSArray{
    ...
    private static function processJsonObject($value,GSArray $gsarr)
    {
        if(!empty($value)){
    ...
    public function getBool($inx){
        $obj = $this->map[$inx];
        if ($obj === null)
            throw new GSException(GSArray::NO_INDEX_EX.$inx);
    ...
    public static function serializeGSArray(GSArray $gsarr){
        $arr = Array();
